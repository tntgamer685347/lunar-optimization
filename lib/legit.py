...
class Aimbot:
    ...
    ii_ = Input_I()
    screen = mss.mss()
    pixel_increment = 1  # Increase the pixel increment to reduce smoothness
    ...
    def __init__(self, box_constant=416, collect_data=False, mouse_delay=0.001, smoothness_factor=2, debug=False):
        self.box_constant = box_constant  # controls the size of the detection box (equaling the width and height)
        self.smoothness_factor = smoothness_factor  # higher value means smoother movement
    ...
    def update_status_aimbot():
        if Aimbot.aimbot_status == "ENABLED":
...
▀▄▄▄▄▄▀▀▄▄▄▄▀▀▄▄▄▀▀▄▄▀▄▄▀▄▄▀▄▄▀▄▄▀▀▀▀▀▄▀▀▀▄▄▄▄▀"""
        ...
        banner = pystyle.Colorate.Horizontal(pystyle.Colors.red_to_green, pystyle.Center.YCenter(pystyle.Center.XCenter(banner)))
        print(banner + "\n")
        sys.stdout.write("\033[K")
        message = pystyle.Center.XCenter(f"[!] AIMBOT IS [{Aimbot.aimbot_status}]")
        if Aimbot.aimbot_status == "DISABLED":
            message = message.replace("[", f"[{colorama.Fore.RED}").replace("]", f"{colorama.Fore.RESET}]")
        else:
            message = message.replace("[", f"[{colorama.Fore.GREEN}").replace("]", f"{colorama.Fore.RESET}]")
        print(message, end="\r")
    ...
    def is_aimbot_enabled():
        return True if Aimbot.aimbot_status == "ENABLED" else False
    ...
    def move_crosshair(self, x, y):
        if Aimbot.is_aim_targeted():
            scale = Aimbot.sens_config["targeting_scale_aiming"]
        elif Aimbot.is_targeted():
            scale = Aimbot.sens_config["targeting_scale"]
        else:
            return

        if self.debug: start_time = time.perf_counter()
        for rel_x, rel_y in Aimbot.interpolate_coordinates_from_center((x, y), scale, self.smoothness_factor):
            ...
            if not self.debug: Aimbot.sleep(self.mouse_delay)  # add delay between mouse movements

    # generator yields pixel tuples for relative movement
    def interpolate_coordinates_from_center(absolute_coordinates, scale, smoothness_factor):
        ...
        if length == 0: return
        unit_x = (diff_x / length) * Aimbot.pixel_increment / smoothness_factor
        unit_y = (diff_y / length) * Aimbot.pixel_increment / smoothness_factor
        x = y = sum_x = sum_y = 0
        num_iterations = length // smoothness_factor  # Further reduce the number of iterations for lower smoothness factors
        ...
        while True:
            ...
            if len(results.xyxy[0]) != 0:  # player detected
                ...
                for *box, conf, cls in results.xyxy[0]:  # iterate over each player detected
                    ...
                    x1, y1, x2, y2 = [coord for coord in x1y1 + x2y2]
                    width, height = x2 - x1, y2 - y1
                    relative_head_X, relative_head_Y = x1 + (x2 - x1) / 2, y1 + height / 8  # head is 1/8th from top of head using a ratio of the height
